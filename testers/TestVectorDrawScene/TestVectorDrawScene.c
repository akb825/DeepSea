	...
	dsAllocator* allocator)
{
	...
	testVectorDrawScene->view = dsView_loadArchive(allocator, "window", testVectorDrawScene->scene,
		NULL, scratchData, &viewSurface, 1, surface->width, surface->height, surface->rotation,
...
int dsMain(int argc, const char** argv)
{
	...
	dsApplication* application = dsSDLApplication_create((dsAllocator*)&applicationAllocator,
		renderer, argc, argv, "DeepSea", "TestVectorDrawScene", dsSDLApplicationFlags_None);
	if (!application)
	{
