...
bool dsOrientedBox2f_transform(dsOrientedBox2f* box, const dsMatrix33f* transform)
{
	...
	dsVector3f halfExtents = {{{box->halfExtents.x, box->center.y, 0}}};

	dsMatrix22f newOrientation;
	dsVector3f newCenter;
	dsVector3f newHalfExtents;
	...
	dsMatrix33_transform(newHalfExtents, *transform, halfExtents);
...
bool dsOrientedBox2d_transform(dsOrientedBox2d* box, const dsMatrix33d* transform)
{
	...
	dsVector3d halfExtents = {{{box->halfExtents.x, box->center.y, 0}}};

	dsMatrix22d newOrientation;
	dsVector3d newCenter;
	dsVector3d newHalfExtents;
...
void dsOrientedBox2f_addPoint(dsOrientedBox2f* box, const dsVector2f* point)
{
	...
	if (dsOrientedBox2_isValid(*box))
	{
		...
		dsVector2_add(localPoint, localPoint, box->center);

		dsAlignedBox2f localBox;
		dsVector2_sub(localBox.min, box->center, box->halfExtents);
		dsVector2_add(localBox.max, box->center, box->halfExtents);

		dsAlignedBox2_addPoint(localBox, localPoint);

		dsAlignedBox2_center(box->center, localBox);
...
bool dsOrientedBox2f_addBox(dsOrientedBox2f* box, const dsOrientedBox2f* otherBox)
{
	...
	if (dsOrientedBox2_isValid(*box))
	{
		...
		dsVector2f corners[DS_ORIENTED_BOX2_CORNER_COUNT];
		...
		for (unsigned int i = 0; i < DS_ORIENTED_BOX2_CORNER_COUNT; ++i)
		{
			...
			dsVector2_add(localCorner, localCorner, box->center);
...
bool dsOrientedBox2d_addBox(dsOrientedBox2d* box, const dsOrientedBox2d* otherBox)
{
	...
	if (dsOrientedBox2_isValid(*box))
	{
		...
		dsVector2d corners[DS_ORIENTED_BOX2_CORNER_COUNT];
...
bool dsOrientedBox2f_corners(dsVector2f corners[], const dsOrientedBox2f* box)
{
	...
	{
		dsVector2f localPoint;
		dsMatrix22_transform(localPoint, box->orientation, corners[i]);
		dsVector2_add(corners[i], localPoint, box->center);
	}
...
bool dsOrientedBox2f_intersects(const dsOrientedBox2f* box, const dsOrientedBox2f* otherBox)
{
	...
	// Test axes of box.
	{
		dsVector2f otherBoxCorners[DS_ORIENTED_BOX2_CORNER_COUNT];
		...
		for (unsigned int i = 0; i < 2; ++i)
		{
			...
			for (unsigned int j = 0; j < DS_ORIENTED_BOX2_CORNER_COUNT; ++j)
			{
	...
	{
		dsVector2f boxCorners[DS_ORIENTED_BOX2_CORNER_COUNT];
...
bool dsOrientedBox2d_intersects(const dsOrientedBox2d* box, const dsOrientedBox2d* otherBox)
{
	...
	// Test axes of box.
	{
		dsVector2d otherBoxCorners[DS_ORIENTED_BOX2_CORNER_COUNT];
	...
	// Test axes of other box.
	{
		dsVector2d boxCorners[DS_ORIENTED_BOX2_CORNER_COUNT];
...
float dsOrientedBox2f_dist2(const dsOrientedBox2f* box, const dsVector2f* point)
{
	...
	return dsAlignedBox2f_dist2(&localBox, point);
}
...
double dsOrientedBox2d_dist2(const dsOrientedBox2d* box, const dsVector2d* point)
{
	...
	return dsAlignedBox2d_dist2(&localBox, point);
}
