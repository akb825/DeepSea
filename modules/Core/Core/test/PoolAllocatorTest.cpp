...
TEST(PoolAllocator, Initialize)
{
	...
	const unsigned int bufferSize = DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT)*chunkCount;
	...
	EXPECT_EQ(DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), allocator.chunkSize);
...
TEST(PoolAllocator, AllocateFree)
{
	...
	EXPECT_EQ(DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), ((dsAllocator*)&allocator)->size);
	...
	EXPECT_EQ(buffer + DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), ptr2);
	EXPECT_TRUE(dsPoolAllocator_validate(&allocator));
	EXPECT_EQ(2U, allocator.head);
	EXPECT_EQ(2U, allocator.freeCount);
	EXPECT_EQ(2U, allocator.initializedCount);
	EXPECT_EQ(2*DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), ((dsAllocator*)&allocator)->size);
	...
	EXPECT_EQ(buffer + 2*DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), ptr3);
	EXPECT_TRUE(dsPoolAllocator_validate(&allocator));
	EXPECT_EQ(3U, allocator.head);
	EXPECT_EQ(1U, allocator.freeCount);
	EXPECT_EQ(3U, allocator.initializedCount);
	EXPECT_EQ(3*DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), ((dsAllocator*)&allocator)->size);
	...
	EXPECT_EQ(buffer + 2*DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), ptr4);
	EXPECT_TRUE(dsPoolAllocator_validate(&allocator));
	EXPECT_EQ(0U, allocator.head);
	EXPECT_EQ(2U, allocator.freeCount);
	EXPECT_EQ(3U, allocator.initializedCount);
	...
	EXPECT_EQ(buffer + 3*DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), ptr6);
	EXPECT_TRUE(dsPoolAllocator_validate(&allocator));
	EXPECT_EQ(SIZE_MAX, allocator.head);
	EXPECT_EQ(0U, allocator.freeCount);
	EXPECT_EQ(4U, allocator.initializedCount);
	EXPECT_EQ(4*DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), ((dsAllocator*)&allocator)->size);
	...
	EXPECT_EQ(buffer + 3*DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT), ptr8);
	EXPECT_TRUE(dsPoolAllocator_validate(&allocator));
	EXPECT_EQ(2U, allocator.head);
	EXPECT_EQ(1U, allocator.freeCount);
	EXPECT_EQ(4U, allocator.initializedCount);
...
TEST(PoolAllocator, ThreadAlloc)
{
	...
	const unsigned int chunkSize = 24;
	const unsigned int bufferSize = DS_ALIGNED_SIZE(chunkSize, DS_ALLOC_ALIGNMENT)*threadCount;
	DS_ALIGN(DS_ALLOC_ALIGNMENT) uint8_t buffer[bufferSize];
