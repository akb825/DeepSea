...
#include "Flatbuffers/HandoffNode_generated.h"
	...
	dsCloseSceneResourcesRelativePathStreamFunction closeRelativePathStreamFunc)
{
	...
	if (!DeepSeaScene::VerifyHandoffNodeBuffer(verifier))
	{
		errno = EFORMAT;
		DS_LOG_ERROR(DS_SCENE_LOG_TAG, "Invalid handoff node flatbuffer format.");
		return nullptr;
	}

	auto fbHandoffNode = DeepSeaScene::GetHandoffNode(data);
	...
	auto node = reinterpret_cast<dsSceneNode*>(dsSceneHandoffNode_create(allocator,
		fbHandoffNode->transitionTime(), itemLists, itemListCount));
