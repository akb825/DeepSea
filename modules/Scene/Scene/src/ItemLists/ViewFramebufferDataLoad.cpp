...
#include "Flatbuffers/ViewFramebufferData_generated.h"
...
extern "C"
dsSceneInstanceData* dsViewFramebufferData_load(const dsSceneLoadContext* loadContext,
	dsSceneLoadScratchData* scratchData, dsAllocator* allocator, dsAllocator* resourceAllocator,
	void*, const uint8_t* data, size_t dataSize)
{
	flatbuffers::Verifier verifier(data, dataSize);
	if (!DeepSeaScene::VerifyViewFramebufferDataBuffer(verifier))
	{
		errno = EFORMAT;
		DS_LOG_ERROR(DS_SCENE_LOG_TAG, "Invalid view framebuffer data flatbuffer format.");
		return nullptr;
	}

	auto fbTransformData = DeepSeaScene::GetViewFramebufferData(data);
	...
	if (!dsSceneLoadScratchData_findResource(
			&resourceType, reinterpret_cast<void**>(&groupDesc), scratchData, groupDescName) ||
		resourceType != dsSceneResourceType_ShaderVariableGroupDesc)
	{
		...
		DS_LOG_ERROR_F(DS_SCENE_LOG_TAG,
			"Couldn't find view framebuffer shader variable group description '%s'.",
			groupDescName);
		return nullptr;
	}

	dsRenderer* renderer = dsSceneLoadContext_getRenderer(loadContext);
	return dsViewFramebufferData_create(
