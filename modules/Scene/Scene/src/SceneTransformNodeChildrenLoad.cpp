...
#include "Flatbuffers/TransformNodeChildren_generated.h"
	...
	void* userData, const uint8_t* data, size_t dataSize)
{
	...
	if (!DeepSeaScene::VerifyTransformNodeChildrenBuffer(verifier))
	{
		errno = EFORMAT;
	...
	auto fbTransformNodeChildren = DeepSeaScene::GetTransformNodeChildren(data);
	const char* nodeName = fbTransformNodeChildren->node()->c_str();
	...
	if (!dsSceneLoadScratchData_findResource(&resourceType, reinterpret_cast<void**>(&node),
		...
		!dsSceneNode_isOfType(node, dsSceneTransformNode_type()))
	{
		// NOTE: ENOTFOUND not set when the type doesn't match, so set it manually.
		errno = ENOTFOUND;
		DS_LOG_ERROR_F(DS_SCENE_LOG_TAG, "Couldn't find transform node '%s'.", nodeName);
		return false;
	}

	auto fbChildren = fbTransformNodeChildren->children();
