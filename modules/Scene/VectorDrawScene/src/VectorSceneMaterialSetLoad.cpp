...
static bool convertStops(dsAllocator* allocator, dsGradientStop*& tempStops, uint32_t& stopCount,
	uint32_t& maxStops,
	const flatbuffers::Vector<flatbuffers::Offset<DeepSeaVectorDrawScene::GradientStop>>* fbStops)
{
	stopCount = 0;
	...
	void*, const uint8_t* data, size_t dataSize)
{
	...
	if (!DeepSeaVectorDrawScene::VerifyVectorMaterialSetBuffer(verifier))
	{
		errno = EFORMAT;
		DS_LOG_ERROR(DS_VECTOR_DRAW_SCENE_LOG_TAG,
	...
	auto fbVectorMaterialSet = DeepSeaVectorDrawScene::GetVectorMaterialSet(data);
